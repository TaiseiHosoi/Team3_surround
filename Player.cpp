...
void Player::Initialize(Model* model, Model* followModel, Model* playerModel)
{
	...
	//color
	blueColor = { 0.2f,1.0f,0.8f,1.0f };
	pinkColor = { 1.0f,0.1f,1.0f,1.0f };
...
#pragma region 攻撃
	...
	if (isAtkDraw == true) {
		...
		if (atkTransform_.position.z > 1000) {
			isAtkDraw = false;
		}
	}
...
#pragma region color
	if (isReversal == false) {
		atkTransform_.SetRimColor(blueColor);
		...
		atkTransform_.SetRimColor(pinkColor);
